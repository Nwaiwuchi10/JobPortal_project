...
const CategoryCarousel = () => {
  ...
  return (
    <div className="w-full px-4 my-10">
      {/* Small & Medium Screens: Use flex-wrap instead of carousel */}
      <div className="flex flex-wrap justify-center gap-2 md:hidden">
        {category.map((cat, index) => (
          <Button
            ...
            onClick={() => searchJobHandler(cat)}
            variant="outline"
            className="rounded-full px-4 py-2 text-xs sm:text-sm truncate"
            title={cat}
          >
            {cat}
          </Button>
        ...
        <Carousel className="w-full max-w-6xl mx-auto">
          <CarouselContent>
            {category.map((cat, index) => (
              <CarouselItem
                key={index}
                className="basis-1/3 md:basis-1/4 lg:basis-1/5 px-2"
              >
                <Button
                  onClick={() => searchJobHandler(cat)}
                  variant="outline"
                  className="w-full px-2 py-3 text-sm lg:text-base rounded-full truncate"
                  title={cat}
                >
                  {cat}
                </Button>
